...
$(function() {
	...
	// 国名が変更されたら発動
	$('select[name="client[prefecture]"]').change(function() {
		
		// 選択されている国のクラス名を取得
		var countyName = $('select[name="client[prefecture]"] option:selected').attr("class");
		console.log(countyName);
		
		// 都市名の要素数を取得
		var count = $('select[name="client[city]"]').children().length;
		
		// 都市名の要素数分、for文で回す
		for (var i=0; i<count; i++) {
			
			var city = $('select[name="client[city]"] option:eq(' + i + ')');
